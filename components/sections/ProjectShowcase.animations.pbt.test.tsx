      ...
      fc.property(
        ...
        (projects) => {
          ...
          for (const article of Array.from(articles)) {
            ...
            // will-change is set dynamically based on prefersReducedMotion
 ...
 * 
 * Property: For any animation in the section, when the user has prefers-reduced-motion
...
describe('Property 11: Motion preference respect', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (projects, prefersReducedMotion) => {
          ...
          // When reduced motion is preferred, articles should still render
          // but with different animation properties (handled by Framer Motion)
          if (articles.length !== projects.length) return false;
  ...
  it('should set opacity to 1 immediately when reduced motion is preferred', () => {
    fc.assert(
      fc.property(
        fc.array(
          fc.record({
        ...
        (projects) => {
          // Mock matchMedia to enable prefers-reduced-motion
          (global as any).matchMedia = vi.fn().mockImplementation((query: string) => ({
          ...
          // All articles should render regardless of motion preference
          return articles.length === projects.length;
        }
      ),
      { numRuns: 20 }
  ...
  it('should disable hover scale effect when reduced motion is preferred', () => {
    fc.assert(
      fc.property(
        fc.array(
          fc.record({
        ...
        (projects) => {
          ...
          // Component should render all articles with reduced motion
          // The hover effect is conditionally applied based on prefersReducedMotion
          return articles.length === projects.length;
        }
      ),
      { numRuns: 20 }
