...
import { Children, type ComponentProps, type FormEvent, type ReactNode, useCallback, useEffect, useId, useMemo, useRef, useState } from "react";
...
import { addDoc, collection, doc, getDoc, getDocs, onSnapshot, query, runTransaction, Timestamp, where } from "firebase/firestore";
...
function NeptuneStagger({
    ...
    const itemVariants = {
        ...
        show: {
            ...
            y: 0,
            filter: "blur(0px)",
            transition: { duration: 1.15, ease: [0.19, 1, 0.22, 1] as [number, number, number, number] },
        },
    };
    ...
    return (
        <motion.div
            ...
            initial="hidden"
            whileInView="show"
            viewport={{ once: once ?? true, amount: 0.08, margin: "20% 0px 20% 0px" }}
        >
            {items.map((child, idx) => (
...
type NavSectionId = "home" | "couple" | "event" | "story" | "gallery" | "rsvp" | "gift" | "wishes";

function TitleDecoration10({ className }: { className?: string }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            version="1.1"
            viewBox="-0.5 -0.5 844 461"
            className={className}
            aria-hidden="true"
            focusable="false"
        ...
        >
            <g>
                <path
                    fill="#5E727B"
                    d="M -0.5,-0.5 C 0.5,-0.5 1.5,-0.5 2.5,-0.5C 141.85,48.3957 281.516,96.729 421.5,144.5C 561.495,96.7255 701.161,48.3921 840.5,-0.5C 841.5,-0.5 842.5,-0.5 843.5,-0.5C 843.5,153.167 843.5,306.833 843.5,460.5C 562.167,460.5 280.833,460.5 -0.5,460.5C -0.5,306.833 -0.5,153.167 -0.5,-0.5 Z"
                />
            </g>
        </svg>
    );
...
export function Neptune1({ config }: Neptune1Props) {
    ...
    const [backgroundReady, setBackgroundReady] = useState(() => !config.sections.hero.enabled);
    const [pageAssetsReady, setPageAssetsReady] = useState(() => !config.sections.hero.enabled);
        ...
        const urls = Array.from(
            new Set(
                [
                    ...
                    "https://ik.imagekit.io/geb6bfhmhx/activid%20web/invitation/template/neptune-1/7.png",
                    "https://ik.imagekit.io/geb6bfhmhx/activid%20web/invitation/template/neptune-1/8.png",
                    "https://ik.imagekit.io/geb6bfhmhx/activid%20web/invitation/template/neptune-1/11.png",
                    "https://ik.imagekit.io/geb6bfhmhx/activid%20web/invitation/template/neptune-1/2.png?updatedAt=1769751311201",
                    "https://ik.imagekit.io/geb6bfhmhx/activid%20web/invitation/template/neptune-1/3.png?updatedAt=1769751313795",
                ].filter(Boolean)
            )
        );
    ...
    }, [config.sections.hero.coverImage, config.sections.hero.enabled, titleBackgroundPhotos]);
    ...
    return (
        ...
        >
            ...
            <audio ref={audioRef} src={audioStreamUrl} loop={config.music.loop} preload="auto" />
            ...
            {contentReady && pageAssetsReady ? (
                <div className="relative z-10">
                    ...
                    {config.sections.quote.enabled ? <QuoteSection text={config.sections.quote.text} /> : null}

                    {config.sections.couple.enabled ? (
                        <CoupleProfileSection
                            couple={config.couple}
                            inviteLine={inviteLine}
                            nextSectionId={config.sections.event.enabled ? "event" : config.sections.story.enabled ? "story" : "gallery"}
                        />
                    ) : null}

                    {config.sections.event.enabled && (
                        <WeddingEventSection id="event" heading={"Wedding Event"} events={events} />
                    )}
                    ...
                    {config.sections.gallery.enabled && (
                        <GallerySectionClassic id="gallery" heading={config.sections.gallery.heading || "Gallery"}>
                            <GalleryGrid photos={config.sections.gallery.photos} uiVariant="classic" />
                        </GallerySectionClassic>
                    )}

                    {config.sections.gift.enabled && (
                        <GiftSectionClassic id="gift" heading={config.sections.gift.heading || "Wedding Gift"}>
                            <GiftBlock
                    ...
                    {config.sections.wishes.enabled && (
                        <WishesSectionClassic id="wishes" heading={config.sections.wishes.heading || "Friends Wishes"}>
                            <WishesFirestore
...
function StarfieldLayer({ density }: { density?: number }) {
    const stars = useMemo(() => {
...
function CoverOverlay({
    ...
    const itemVariants = {
        ...
        show: {
            opacity: 1,
            y: 0,
            transition: { duration: 0.75, ease: [0.19, 1, 0.22, 1] as [number, number, number, number] },
        },
    };
    ...
    const imageVariants = {
        ...
        show: {
            ...
            y: 0,
            scale: 1,
            transition: { duration: 0.95, ease: [0.19, 1, 0.22, 1] as [number, number, number, number] },
        },
    };
    ...
    return (
        <div className="relative h-screen w-full overflow-hidden bg-[#4E5C6C] text-[#2E343A]">
            <motion.div
                ...
                style={{
                    backgroundImage: "linear-gradient(120deg, #8FA2BA, #5B6A7C, #3E4A58, #6D7C8F)",
            ...
            <motion.div
                ...
                transition={{ duration: 6.2, repeat: Infinity, ease: "easeInOut", repeatDelay: 2.4 }}
            />
                        ...
                        <div className="rounded-[38px] overflow-hidden border border-black/15 bg-[#E7E4DC]">
                            <motion.div variants={imageVariants} className="px-4 pt-4">
                                <div className="relative w-full aspect-[4/3] rounded-t-[999px] rounded-b-none border border-black/15 bg-black">
                                    ...
                                    <img
                                        src="https://ik.imagekit.io/geb6bfhmhx/activid%20web/invitation/template/neptune-1/8.png"
                                        alt=""
                            ...
                            <div className=" pt-14 pb-9 text-center">
                                <motion.h1 variants={itemVariants} className={`${neptuneScript.className} mt-4 text-4xl leading-none text-[#546058]`}>
                                    {couple.bride.firstName} & {couple.groom.firstName}
                                </motion.h1>

                                <motion.p variants={itemVariants} className="mt-6 text-sm tracking-[0.35em] uppercase text-[#4A5257]">
                                    {heroDate}
                                </motion.p>

                                <motion.p variants={itemVariants} className="mt-12 text-sm leading-relaxed text-[#4A5257]">
                                    Kepada Yth. Bapak/Ibu/Saudara/i
                                    <br />
                                </motion.p>
                                <motion.p variants={itemVariants} className="mt-2 text-3xl leading-none tracking-wide text-[#2E343A] font-body">
                                    {guestName}
                                </motion.p>
                                <motion.button
                                ...
                                >
                                    <motion.span
                                        ...
                                        transition={{ duration: 1.15, repeat: Infinity, repeatDelay: 4.2, ease: "easeInOut" }}
                                    />
                                    <span className="relative z-10 text-sm font-body tracking-wide">Buka Undangan</span>
                                </motion.button>
                            </div>
                        </div>

                        <img
                            src="https://ik.imagekit.io/geb6bfhmhx/activid%20web/invitation/template/neptune-1/7.png"
                            alt=""
...
function TitleCountdownSection({
    ...
    return (
        <section id={id} className="relative isolate scroll-mt-24 min-h-screen overflow-hidden bg-[#F3F1EC] text-[#2E343A]">
                    ...
                    <AnimatePresence initial={false}>
                        {activePhoto ? (
                            <motion.div
                                ...
                                animate={{ opacity: 1 }}
                                exit={{ opacity: 0 }}
                                transition={{ duration: 1.2, ease: [0.19, 1, 0.22, 1] as [number, number, number, number] }}
                                className="absolute inset-0"
                            >
                                <motion.div
                                    initial={{ scale: 1.06 }}
            ...
            <div className="absolute inset-0 z-10 pointer-events-none">
                ...
                <img
                    src="https://ik.imagekit.io/geb6bfhmhx/activid%20web/invitation/template/neptune-1/2.png?updatedAt=1769751311201"
                    alt=""
                ...
                />
                <img
                    src="https://ik.imagekit.io/geb6bfhmhx/activid%20web/invitation/template/neptune-1/3.png?updatedAt=1769751313795"
                    alt=""
            ...
            <div className="relative z-30 min-h-screen flex flex-col items-center justify-end text-center px-6 pb-32 pt-18">
                <NeptuneStagger className="w-full max-w-sm" baseDelay={0.15}>
                    <p className={`${neptuneSerif.className} text-sm tracking-[0.28em] uppercase text-[#667078]`}>
                        The Wedding Of
                    </p>
                    <h2 className={`${neptuneSerif.className} mt-4 text-5xl leading-none text-[#586057]`}>{coupleLabel}</h2>
                    <p className="mt-4 text-lg tracking-wide text-[#5C666F] font-body">{date}</p>
                    <p className="mt-10 text-xs tracking-[0.45em] uppercase text-[#7A838B] font-body">SAVE THE DATE</p>
                    <div className="mt-6">
...
function GallerySectionClassic({
    ...
    return (
        <section id={id} className="relative scroll-mt-24 overflow-hidden bg-[#F3F1EC] text-[#2E343A] px-6 py-16">
            <div className="absolute inset-0 pointer-events-none">
                ...
                <div className="rounded-[34px] border border-black/10 bg-white/70 shadow-[0_24px_60px_rgba(0,0,0,0.10)]">
                    <div className="p-10">
                        <NeptuneStagger baseDelay={0.1}>
                            <h3 className={`${neptuneSerif.className} text-4xl text-[#6B7480] text-center`}>{heading}</h3>
        ...
        <section className="relative overflow-hidden bg-[#5F737B] text-white px-6 py-4">
            ...
            <div className="relative z-10 py-10 flex items-center justify-center">
                <NeptuneStagger className="max-w-3xl text-center" baseDelay={0.1}>
                    ...
                    <p className="mt-10 text-xl md:text-2xl leading-relaxed text-white/95 whitespace-pre-line">{main}</p>
                    {citation ? <p className="mt-10 text-2xl text-white/90">{citation}</p> : null}
                </NeptuneStagger>
            </div>
        </section>
    );
...
function CountdownRow({ targetDate }: { targetDate: string }) {
    ...
    return (
        <NeptuneStagger className="grid grid-cols-4 gap-3" baseDelay={0.08} staggerStep={0.22}>
            {cells.map((c) => (
                <div key={c.label}>
                    <div className="rounded-2xl border border-white/20 bg-[#5E727B] px-2 py-3 shadow-[0_10px_35px_rgba(0,0,0,0.12)]">
                        <div className={`${neptuneSerif.className} text-3xl leading-none text-white`}>{c.value}</div>
                        <div className="mt-2 text-[12px] leading-none text-white/85 font-body">{c.label}</div>
                    </div>
                </div>
            ))}
        </NeptuneStagger>
...
function FloralCorner({
    ...
    return (
        <NeptuneIdle className={`absolute ${className}`} amplitude={10} duration={9.5}>
            <svg viewBox="0 0 220 220" style={{ transform }} fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M34 196c30-25 54-60 69-101 8-21 14-44 18-71"
                    stroke="rgba(27,55,80,0.55)"
                    strokeWidth="2"
                    strokeLinecap="round"
                />
                <path
                    d="M62 168c18-10 40-36 46-58"
                    stroke="rgba(27,55,80,0.45)"
                    strokeWidth="2"
                    strokeLinecap="round"
                />
                <path
                    d="M72 186c-14-8-24-18-30-32 12 4 26 6 41 6-5 11-8 18-11 26Z"
                    fill="rgba(120,145,170,0.65)"
                />
                <path
                    d="M104 156c-10-7-18-17-22-30 11 4 23 6 37 6-5 10-9 16-15 24Z"
                    fill="rgba(86,118,150,0.65)"
                />
                <path
                    d="M148 44c-9 4-18 12-24 22 12-3 22-9 31-18-3-2-5-4-7-4Z"
                    fill="rgba(180,150,90,0.55)"
                />
                <path
                    d="M168 70c-12 2-22 9-29 20 12 0 24-4 36-12-3-3-5-6-7-8Z"
                    fill="rgba(180,150,90,0.45)"
                />
            </svg>
        </NeptuneIdle>
    );
...
function IconEnvelope() {
    return (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            />
            <path d="M5.2 7.2 12 12.3l6.8-5.1" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" />
        </svg>
    );
}
...
function CoupleProfileCard({
    ...
    return (
        <section id={id} className="relative min-h-screen overflow-hidden bg-[#F3F1EC] text-[#2E343A] px-6 py-16">
            <div className="absolute inset-0 pointer-events-none">
                <div className="absolute inset-0 opacity-[0.26] bg-[radial-gradient(circle_at_30%_10%,rgba(0,0,0,0.06),transparent_55%),radial-gradient(circle_at_80%_70%,rgba(0,0,0,0.05),transparent_60%)]" />
                <FloralCorner className="left-0 top-20 w-56 opacity-50" flipX />
                <FloralCorner className="right-0 bottom-0 w-60 opacity-55" />
            </div>

            <NeptuneStagger className="relative z-10 max-w-md mx-auto text-center" baseDelay={0.12}>
                {showAnd ? (
                    <div className="flex items-center justify-center gap-4 text-xs tracking-[0.35em] uppercase text-[#6B747C]">
                        ...
                        <span className={`${neptuneSerif.className} text-sm tracking-[0.32em]`}>AND</span>
                        <div className="h-px w-16 bg-[#9AA3AB]" />
                    </div>
                ) : null}
                ...
                {intro ? (
                    <div className="mt-4 flex justify-center">
                        <motion.img
                            src="https://ik.imagekit.io/geb6bfhmhx/activid%20web/invitation/template/neptune-1/11.png"
                            alt=""
                ...
                {intro ? <p className="mt-6 text-lg leading-relaxed text-[#6A737B]">{intro}</p> : null}

                <div className={intro ? "mt-10" : "mt-14"}>
                    <ArchedPortrait photo={photo} alt={nameScript} />
                </div>

                <p className={`${neptuneScript.className} mt-10 text-5xl leading-none text-[#8D8A7E]`}>{nameScript}</p>
                <p className={`${neptuneSerif.className} mt-5 text-4xl leading-none text-[#586057]`}>{nameFull}</p>
                <p className="mt-6 text-xl font-bold text-[#5B646D] font-body">{city}</p>
            ...
            <button
                type="button"
                onClick={onNext}
                className="absolute bottom-6 right-6 h-12 w-12 rounded-full bg-white/80 border border-black/10 shadow-sm flex items-center justify-center text-[#5B646D] hover:bg-white transition"
                aria-label="Next"
            >
                <IconArrowRight />
            </button>
...
function ArchedPortrait({ photo, alt }: { photo: string; alt: string }) {
    return (
        <div className="flex justify-center">
            <div className="w-[260px] sm:w-[290px]">
                <div className="rounded-t-[999px] rounded-b-none border border-[#8496AC] bg-[#F6F4EF] p-[7px] shadow-[0_16px_50px_rgba(0,0,0,0.10)]">
                    <div className="relative aspect-[4/4.3] rounded-t-[999px] rounded-b-none overflow-hidden border border-[#8496AC] bg-white">
                        <Image src={photo} alt={alt} fill sizes="320px" className="object-cover" unoptimized />
                    </div>
                </div>
            </div>
        </div>
...
function WeddingEventSection({
    ...
    id: NavSectionId;
    heading: string;
    events: Array<{ key: string; title: string; date: string; time: string; venue: string; address: string; mapUrl: string }>;
}) {
    ...
    return (
        <section id={id} className="relative scroll-mt-24 overflow-hidden bg-[#4E5C6C] text-white px-6 pt-18 pb-20">
            <div className="absolute inset-0 bg-linear-to-b from-[#59677A] via-[#4E5C6C] to-[#3E4A58]" />
            <div className="absolute inset-0 pointer-events-none">
                <FloralCorner className="left-0 bottom-0 w-64 opacity-55" flipX />
                <FloralCorner className="right-0 bottom-0 w-56 opacity-55" />
            </div>
            ...
            <div className="relative z-10 max-w-xl mx-auto">
                <NeptuneReveal direction="up" width="100%" delay={0.18}>
                    <div className="flex items-start gap-4">
                        <div className="leading-none">
                            <div className={`${neptuneSerif.className} text-6xl md:text-7xl text-white/95`}>{topWord}</div>
                            <div className={`${neptuneScript.className} -mt-2 text-5xl md:text-6xl text-white/90`}>{scriptWord}</div>
                        </div>
                ...
                <div className="mt-10 space-y-10">
                    {events.map((e, idx) => (
                        <NeptuneReveal key={e.key} width="100%" direction="up" delay={0.35 + idx * 0.25}>
                            <EventCardClassic
                                title={e.title}
                                date={e.date}
                                time={e.time}
    ...
    return (
        <div className="relative rounded-[28px] bg-white text-[#2E343A] shadow-[0_26px_70px_rgba(0,0,0,0.35)]">
            ...
            <div className="relative rounded-[28px] border border-black/10 px-8 py-12">
                <h4 className={`${neptuneSerif.className} text-6xl leading-none text-[#6B756B] text-center`}>{title}</h4>
                ...
                {!isLinkOnly ? (
                    <div className="mt-7 text-center">
                        {date ? (
                            <p className="text-md md:text-[38px] leading-none tracking-[0.14em] uppercase text-[#3A3F45] font-body">
                                {date}
                            </p>
                        ) : null}
                        ...
                        <div className="mt-7 flex items-center justify-center gap-3 text-[#6B747C]">
                            {time ? (
                                <>
                                    <IconClock />
                                    <p className="text-[22px] tracking-[0.22em] uppercase font-body">{time}</p>
                                </>
                            ) : null}
                        </div>

                        {venue ? <p className="mt-6 text-md font-semibold text-[#5A6168] font-body">{venue}</p> : null}
                        {address ? <p className="mt-2 text-xl leading-tight text-[#5A6168] font-body whitespace-pre-line">{address}</p> : null}
                    </div>
                ) : null}
                    ...
                    <div className="mt-10 flex justify-center">
                        ...
                        >
                            ...
                            <span className="text-md font-semibold font-body">
                                {title.toLowerCase().includes("live") || title.toLowerCase().includes("stream") ? "Open Link" : "Link Map"}
                            </span>
                        </a>
                    </div>
                ) : null}
...
function StorySectionClassic({
    ...
    return (
        <section id={id} className="relative scroll-mt-24 overflow-hidden bg-[#F3F1EC] text-[#2E343A] px-6 py-16">
            <div className="absolute inset-0 pointer-events-none">
                <FloralCorner className="left-0 top-12 w-56 opacity-40" flipX />
                <FloralCorner className="right-0 top-0 w-52 opacity-35" />
            </div>
                        ...
                        <div className="rounded-[28px] border border-black/10 bg-white/70 overflow-hidden">
                            <div className="relative px-8 pt-10 pb-10 text-center">
                                ...
                                <NeptuneReveal direction="up" width="100%" delay={0.15}>
                                    <p className={`${neptuneSerif.className} text-4xl text-[#667078]`}>{coupleLabel}</p>
                                </NeptuneReveal>
                                <NeptuneReveal direction="up" width="100%" delay={0.35}>
                                    <p className={`${neptuneScript.className} -mt-2 text-5xl text-[#5E6A78]`}>{heading}</p>
                                </NeptuneReveal>
                                    ...
                                    <div className="mt-8">
                                        <div className="rounded-t-[999px] border border-[#6E7B6E] bg-[#F6F4EF] p-[7px] shadow-[0_14px_40px_rgba(0,0,0,0.12)]">
                                            <div className="relative aspect-[4/5] rounded-t-[999px] overflow-hidden border border-[#6E7B6E] bg-white">
                                                <Image src={photo} alt="Story" fill sizes="360px" className="object-cover" unoptimized />
                                            </div>
                                        </div>
                                    </div>
                                </NeptuneReveal>
                                    ...
                                    {stories.map((s, idx) => (
                                        ...
                                        >
                                            <div className="text-center">
                                                <p className="text-xs tracking-[0.28em] uppercase text-[#6B747C] font-body">{s.date}</p>
                                                <p className="mt-5 text-[15px] leading-relaxed text-[#6B747C] whitespace-pre-line">{s.description}</p>
...
function ClassicFlourishDivider() {
    return (
        <motion.svg
            width="240"
            height="34"
...
function GalleryGrid({ photos, uiVariant }: { photos: string[]; uiVariant?: "default" | "classic" }) {
    ...
    return (
        <>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {display.map((p, idx) => (
                    <NeptuneReveal key={`${p}-${idx}`} delay={0.35 + idx * 0.11} width="100%">
                        <button
                            type="button"
                            onClick={() => {
                                setDirection(1);
    ...
    return (
        <div className="space-y-6">
            {description ? (
                ...
                >
                    <p className={isClassic ? "text-sm text-[#6B747C] text-center whitespace-pre-line" : "text-sm text-cyan-100/70 text-center whitespace-pre-line"}>
                        {description}
                    </p>
                </div>
            ) : null}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {bankAccounts.map((b, idx) => (
                    <NeptuneReveal key={`${b.bankName}-${idx}`} direction="up" width="100%" delay={0.35 + idx * 0.45}>
                        <div
                            className={
                                isClassic
                        ...
                        >
                            ...
                            <p className={isClassic ? "mt-4 text-sm font-body text-[#2E343A]" : "mt-4 text-sm font-body text-white/90"}>
                                {b.accountHolder}
                            </p>
                            <p className={isClassic ? "mt-2 text-2xl tracking-wide text-[#2E343A]" : "mt-2 text-2xl tracking-wide text-white"}>
                                {b.accountNumber}
                            </p>
                            <button
                                type="button"
                                ...
                                className={
                                    isClassic
                                        ? "mt-6 inline-flex items-center justify-center rounded-none px-6 py-3 border border-black/15 bg-white hover:bg-black/5 transition w-full"
                          ...
                          {isGiftDialogOpen ? (
                              ...
                              >
                                  ...
                                  >
                                      ...
                                      <h4 className={`mt-3 ${neptuneScript.className} text-4xl leading-none text-[#2B2424]`}>
                                          Exclusive Discount
                                      </h4>
    ...
    return (
        <div className="space-y-6">
            <div className="rounded-3xl border border-white/12 bg-white/8 backdrop-blur p-7">
                {submitted ? (
                    <div className="text-center">
                        <p className="text-xs tracking-[0.35em] uppercase text-cyan-100/60 font-body">{inviteeName ?? "Tamu"}</p>
                    ...
                    <div>
                        <p className="text-center text-sm text-cyan-100/70 whitespace-pre-line">
                            {inviteeName
                                ? description || `Silakan konfirmasi kehadiran sebelum ${rsvpDeadline}`
                        ...
                        <form onSubmit={submit} className="mt-6 space-y-4">
                            <div className="rounded-2xl border border-white/12 bg-white/6 px-4 py-3">
                                <p className="text-xs tracking-[0.35em] uppercase text-cyan-100/60 font-body">Nama</p>
                                <p className="mt-2 text-sm font-body text-white/90">{inviteeName ?? "Tamu"}</p>
                            </div>
                            ...
                            {status === "error" ? (
                                <p className="text-center text-xs text-red-300">Terjadi kesalahan. Coba lagi ya.</p>
                            ) : null}
    ...
    const submit = async () => {
        ...
        try {
            ...
            if (err instanceof Error && err.message === "already-posted") {
                ...
                setHasPosted(true);
                setError((withAttendance ?? true) ? "Kamu sudah mengirim konfirmasi & ucapan. Terima kasih!" : "Kamu sudah mengirim ucapan. Terima kasih!");
            } else {
        ...
        <div className="space-y-6">
            {showForm ? (
                ...
                >
                    {!inviteeName ? (
                        <div className="text-center">
                            <p className={isClassic ? "text-sm text-[#6B747C]" : "text-sm text-cyan-100/70"}>
                                {showAttendance
                        ...
                        <div className="text-center">
                            ...
                            <p className={isClassic ? "mt-4 text-sm text-[#2E343A]" : "mt-4 text-sm text-white/90"}>
                                {thankYouMessage}
                            </p>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            <div className="text-center">
                                ...
                                <p className={isClassic ? "mt-2 text-sm font-body text-[#2E343A]" : "mt-2 text-sm font-body text-white/90"}>
                                    {inviteeName}
                                </p>
                            </div>
            ...
            {showList ? (
                <div className={isClassic ? "rounded-2xl border border-black/10 bg-white/70 backdrop-blur" : "space-y-3"}>
                    {wishes.length === 0 ? (
                        <div
                            className={
                                isClassic
                        ...
                        <div className="px-6">
                            {visibleWishes.map((w, idx) => (
                                <div key={w.id} className={`py-6 ${idx === 0 ? "" : "border-t border-black/10"}`}>
                                    <div className="flex items-start justify-between gap-4">
                                        <div>
                                            <p className="text-sm font-semibold text-[#D16D5A]">{w.name}</p>
                                    ...
                                    <p className="mt-3 text-sm text-[#3A3F45] whitespace-pre-line">{w.message}</p>
                                </div>
                            ))}
                        ...
                        <div className="space-y-3">
                            {visibleWishes.map((w, idx) => (
                                <NeptuneReveal key={w.id} width="100%" direction="up" delay={0.25 + idx * 0.16}>
                                    <div className="rounded-3xl border border-white/12 bg-white/6 backdrop-blur p-7">
                                        <div className="flex items-start justify-between gap-4">
                                            <div>
                                                <p className="text-xs tracking-[0.35em] uppercase text-cyan-100/60 font-body">{w.name}</p>
        ...
        <footer className="relative mt-8 overflow-hidden bg-[#0b0c1a] border-t border-white/5">
            ...
            <div className="absolute inset-0 pointer-events-none">
                ...
                >
                    {stars.map((s, idx) => (
                        <motion.span
                            ...
                            animate={{ opacity: [s.opacity, Math.min(1, s.opacity + 0.6), s.opacity], scale: [1, 1.35, 1] }}
                ...
                <motion.div
                    ...
                    transition={{ duration: 3.1, repeat: Infinity, ease: "easeInOut", repeatDelay: 4.2 }}
                    style={{ transform: "rotate(18deg)" }}
                />
            </div>
            ...
            <div className="relative px-6 py-12 md:py-14">
                <div className="max-w-3xl mx-auto text-center text-white">
                    <NeptuneReveal direction="up" width="100%" delay={0.25} className="flex justify-center">
                    ...
                    <NeptuneReveal direction="up" width="100%" delay={0.75}>
                        <h3
                            className={`mt-4 ${neptuneScript.className} text-5xl md:text-6xl bg-linear-to-r from-indigo-400 via-purple-300 to-cyan-300 bg-clip-text text-transparent bg-size-[200%_auto] animate-[gradient_4s_linear_infinite]`}
