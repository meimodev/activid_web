...
export function TitleSection({ couple, date, heading }: TitleSectionProps) {
  ...
  <h2 className="font-heading text-sm uppercase tracking-[0.4em] mb-6 text-wedding-text-light text-center font-semibold drop-shadow-sm">
  {heading}
  </h2>
  </FloatingParallax>
  </RevealOnScroll>
  ...
  <p className="font-heading text-xl tracking-[0.3em] text-wedding-dark border-t border-b border-wedding-accent/40 py-6 inline-block px-16">
  {date}
  </p>
  </div>
  </FloatingParallax>
...
export function EventSection({ events, heading }: EventSectionProps) {
  ...
  <a href={e.mapUrl} target="_blank" className="inline-block mt-8 px-8 py-3 border border-wedding-accent text-wedding-text hover:bg-wedding-accent hover:text-white transition-all duration-300 rounded-sm text-xs uppercase tracking-[0.2em]">
  View Map
  </a>
  </div>
  </div>
...
export function GiftSection({ bankAccounts, heading, description }: GiftSectionProps) {
  ...
  <div className="grid gap-8 font-heading">
