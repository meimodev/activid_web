          ...
          >
            <div className="flex items-start gap-6 ">
              <div className="shrink-0">
                <p className="font-poppins text-[14px] tracking-[0.28em] uppercase text-wedding-on-dark/85 ">
                  {isBirthday ? "Birthday" : "Wedding"}
                </p>
                <p className="mt-3 font-poppins-bold text-[56px] leading-none text-wedding-on-dark ">
                  Event
                </p>
              </div>

              <div className="mt-10 flex w-full items-center gap-3">
                <div className="h-2 w-2 rounded-full bg-wedding-accent-2" />
                <div className="h-px flex-1 bg-wedding-on-dark/40" />
              </div>
            </div>
          </RevealOnScroll>
          ...
          <div className="mt-14 space-y-10">
            {cards.map((data, idx) => (
              <RevealOnScroll
                ...
                width="100%"
              >
                <div className="relative rounded-[34px] bg-wedding-bg/92 px-8 py-12 text-center text-wedding-dark shadow-[0_18px_55px_rgba(0,0,0,0.25)] ring-1 ring-wedding-on-dark/15 ">
                  <div className="pointer-events-none absolute inset-0">
                    <div className="absolute inset-0 opacity-25 bg-[url('https://www.transparenttextures.com/patterns/paper-fibers.png')]" />
                  </div>

                  <p className="relative font-poppins-bold text-[18px] tracking-[0.28em] uppercase text-wedding-accent-2 ">
                    {data.title}
                  </p>
                  ...
                  <motion.div
                    ...
                    style={{
                      backgroundImage: `url(${overlayAssets.cardEventDivider})`,
                    }}
                    animate={{ y: [0, 2, 0] }}
                    transition={{ duration: 5.8, repeat: Infinity, ease: "easeInOut" }}
                  />

                  <p className="relative mt-6 font-poppins-bold text-[18px] text-wedding-dark ">
                    {formatInvitationDateLong(data.date)}
                  </p>
                  <p className="relative mt-2 font-poppins text-[16px] text-wedding-dark/70">
                    {formatInvitationTime(data.date)}
                  </p>
                  <p className="relative font-poppins text-[16px] text-wedding-dark/70">
                    {data.venue}
                  </p>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="relative mt-5 uppercase inline-flex min-w-[210px] items-center justify-center rounded-full bg-wedding-accent-2 px-10 py-3 font-poppins-bold text-[12px] tracking-[0.28em] text-wedding-on-accent-2 shadow-[0_14px_35px_color-mix(in_srgb,var(--invitation-accent-2)_30%,transparent)] transition-colors hover:bg-wedding-accent-2/85"
                  >
                    Link Map
                  </a>
                </div>
